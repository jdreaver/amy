...
data PrelimDecision a
  = PrelimFailure
  | PrelimSuccess a
  | PrelimIfEq Access Con (PrelimDecision a) (PrelimDecision a)
  deriving (Show, Eq)
...
compileMatch' allmrules = compileFail (Neg []) allmrules

compileFail :: TermD -> Match a -> PrelimDecision a
compileFail _ [] = PrelimFailure
compileFail dsc ((pat1, rhs1):rulesrest) = match pat1 Obj dsc [] [] rhs1 rulesrest

compileSucceed :: [(Con, [TermD])] -> [([Pat], [Access], [TermD])] -> a -> [(Pat, a)] -> PrelimDecision a
compileSucceed _ [] rhs _ = PrelimSuccess rhs
...
match :: Pat -> Access -> TermD -> [(Con, [TermD])] -> [([Pat], [Access], [TermD])] -> a -> [(Pat, a)] -> PrelimDecision a
...
match (PCon pcon pargs) obj dsc ctx work rhs rules =
  ...
  in
    case staticmatch pcon dsc of
      Yes -> succeed'
      No -> compileFail' dsc
      Maybe' -> PrelimIfEq obj pcon succeed' (compileFail' (addneg dsc pcon))
...
switchify PrelimFailure = Failure
switchify (PrelimSuccess x) = Success x
switchify (PrelimIfEq acc con successDec failDec) =
  let
...
collectSwitch acc failDec otherDecs =
  case failDec of
    PrelimIfEq acc' con' successDec' failDec' ->
      if acc == acc'
